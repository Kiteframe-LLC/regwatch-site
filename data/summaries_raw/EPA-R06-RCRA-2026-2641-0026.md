...
The controller-selected document, `EPA-R06-RCRA-2026-2641-0026`, is titled "Hazardous Waste Management System: Identification and Listing of Hazardous Waste; Correction and Extension of Comment Period; Correction." Its text says EPA is correcting an incorrect docket number in a March 26, 2026 proposal and extending the comment period through May 20, 2026.

The correction notice explicitly points back to the underlying substantive proposal published on March 26, 2026 at `91 FR 14666`. In the available docket record, that substantive NPRM is `EPA-R06-RCRA-2026-2641-0001`, which proposes to grant WRB Refining LP in Borger, Texas a continuous delisting for stormwater tank solids. The existing override file now aligns this docket so that `subject_document_id` and `summary_source_document_id` point to `EPA-R06-RCRA-2026-2641-0001`, while `comment_document_id` remains `EPA-R06-RCRA-2026-2641-0026`.

The available record for the substantive NPRM includes the preamble text and metadata for supporting items `EPA-R06-RCRA-2026-2641-0002` ("WRB Petition Evaluation") and `EPA-R06-RCRA-2026-2641-0003` ("Sampling and Analysis Plan Stormwater Solids Delisting Petition Project"). The cached manifest at `data/attachment_fetch_manifests/EPA-R06-RCRA-2026-2641-0001.json` validates successfully and shows `missing_required_doc_ids: []`. Those two supporting PDFs were previously marked optional and failed download with `http_403`, so they do not block a defensible first-round summary.

The substantive NPRM itself contains the core reasoning spine: it identifies the petitioned waste, explains how the waste is generated, reports that EPA considered eight acceptable sample results, states that EPA modeled landfill disposal using DRAS, and proposes conditions involving testing, annual volume limits, reporting, process-change notice, and reopener language.

**Inference**

The selected document is not itself the analytical basis for public comment. It is a procedural correction notice that keeps the comment channel open for the underlying NPRM. The meaningful review therefore depends on the March 26 proposal, which provides a partial but usable analytical spine.

**Why that matters**

The highest-value public comments should focus on whether EPA adequately supports the underlying continuous delisting proposal, not on the clerical correction itself. The useful questions are whether EPA has shown enough to justify an ongoing annual exclusion from a limited sample record, whether the verification conditions are specific enough, and whether the disposal and reopener conditions are clearly bounded.
...
**Procedural relevance:** Core regulatory action that the correction notice leaves in place
...
**Procedural relevance:** Identifies the empirical basis for the proposal now under the extended comment period
**Support type:** preamble explanation
...
The selected document corrects the docket number for the underlying March 26, 2026 proposal and extends the comment period. The substantive proposal that remains open for comment would grant WRB Refining LP in Borger, Texas a continuous delisting for up to 700 cubic yards per year of F037 stormwater tank solids removed from four wastewater-system tanks. EPA says the waste is listed because of mixture and derived-from rules tied to refinery oily wastewater management, but concludes from petitioner-supplied data and DRAS modeling that the waste does not warrant continued listing if specified conditions are met. Those conditions include TCLP-based verification testing before disposal, a 700-cubic-yard annual volume cap, annual reporting of testing and disposal volume, process-change notification, and a reopener if later information shows exceedances or other relevant environmental data.

### Inference

The correction notice matters procedurally because it keeps the comment window open and tells readers where to file comments, but it does not materially alter the substantive regulatory choice. The practical issue for commenters is whether EPA has adequately justified converting a prior one-time delisting into an ongoing annual delisting, and whether the monitoring conditions are robust enough to address variability rather than assuming the future will resemble the sampled past.
...
This is a procedurally normal NPRM sequence with a follow-on correction notice. The correction notice fixes the docket number and extends the public comment period, while leaving the underlying proposal open for substantive review.
